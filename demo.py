...
bot_username = "baesdaily"
# bot_secret_key = "c705faf0-0083-4f42-9294-6abf7ea8f42a"
bot_secret_key = "d8e91668-4d59-4e5d-a4e8-0a46b380be74"
...
def run_every_10_seconds():
	...
	#record = db(user="")
	#now = now.replace(hour=8, minute=0, second=1, microsecond=0)
	print now;
	db = Base(schedule_table_name)
	if db.exists():
		db.open()
		...
		try:
			if schedule_time==AM8:
				...
				if now >= today8am0min and now <= today8am1min:
					...
					send_daily_message(kik, user, chat_id)
	...
	if message_sent_already == True:
		print "One matched and hence waiting";
		sleep(4);
		message_sent_already = False;

def send_daily_message(kik, user_id, chat_id):
	print "Sending message to user -> " + user_id + " " + chat_id;
	...
	for row in csv_file:
		if counter==todays_day:
			...
			if '.jpg' in media_file:
				...
				kik.send_messages([PictureMessage(to=user_id,chat_id=chat_id,pic_url=media_file_location),TextMessage(to=user_id,chat_id=chat_id,body=daily_smoke_show_message,keyboards=[SuggestedResponseKeyboard(to=None,hidden=False,responses=[TextResponse('MEET LOCALS'),TextResponse('SETTINGS'),TextResponse('SHARE THIS BAE')])])])
			elif '.mp4' in media_file:
				media_file_location = "http://107.170.21.148/videos/"+media_file
				kik.send_messages([VideoMessage(to=user_id,chat_id=chat_id,video_url=media_file_location),TextMessage(to=user_id,chat_id=chat_id,body=daily_smoke_show_message,keyboards=[SuggestedResponseKeyboard(to=None,hidden=False,responses=[TextResponse('MEET LOCALS'),TextResponse('SETTINGS'),TextResponse('SHARE THIS BAE')])])])
			break
		else:
			counter+=1
...
if __name__ == "__main__":
	schedule.every(57).seconds.do(run_every_10_seconds)
